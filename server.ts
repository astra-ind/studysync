...
async function startServer() {
  ...
  app.post("/api/scheduler/generate", async (req, res) => {
    try {
      const { prompt, currentLocalDate, timezone, userName, customApiKey, personalContext } = req.body;
      
      if (!prompt) {
        return res.status(400).json({ error: "Prompt is required" });
      }

      // Initialize custom Gemini client if key is provided, or fall back to system key
      const apiKeyToUse = customApiKey || process.env.GEMINI_API_KEY;
      if (!apiKeyToUse) {
        return res.status(400).json({ 
          error: "Gemini API key is missing. Please save your Gemini API key in the AI Coach settings panel to utilize independent AI scheduling." 
        });
      }
